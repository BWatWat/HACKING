...
function MakeHTML(input, Actual) {
  ...
  var Output = '<b><span class="dcon"><span id="' + AreaID + '-attempts">[#] [#] [#] [#] [#]</span> ATTEMPTS REMAIN BEFORE TERMINAL LOCK OUT</b></span>' +
...
//onclick="CheckPassword(\''+input[i]+'\',\''+Actual+'\',\''+Matches+'\',\''+AreaID+'\');"
  ...
  return '<div class="modal">'+Output+'</div>';
}
...
function CheckPassword(Given, Actual, Matches, AreaID) {
  ...
  if (Given == Actual) {
	...
	else {$('.dcon').removeClass('dcon');
    matchess.html(matchess.html() + '<br><span class="dcon"><span>' + Matches + '</span> / ' + Actual.length + '  Matched [' + Given + ']</span>');}
    return false;
  }
}
